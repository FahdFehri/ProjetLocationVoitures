...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (move_uploaded_file($_FILES['image']['tmp_name'], $target)) {
        $sql = "INSERT INTO Voitures (Marque, Modèle, Année, Immatriculation, Disponibilité, Image) VALUES (?, ?, ?, ?, 1, ?)";
        $stmt = $pdo->prepare($sql);
        $stmt->execute([$marque, $modele, $annee, $immatriculation, $image]);
        ...
        <div class="form-overlay">
            ...
            <form action="ajouter_voiture.php" method="POST" enctype="multipart/form-data">
                <div class="form-group">
                    ...
                    <input type="text" id="marque" name="marque" required class="input">
                </div>
                <div class="form-group">
                    <label for="modele">Modèle:</label>
                    <input type="text" id="modele" name="modele" required class="input">
                </div>
                <div class="form-group">
                    <label for="annee">Année:</label>
                    <input type="number" id="annee" name="annee" required class="input">
                </div>
                <div class="form-group">
                    <label for="immatriculation">Immatriculation:</label>
                    <input type="text" id="immatriculation" name="immatriculation" required class="input">
                </div>
                <div class="form-group">
                    <label for="image">Image:</label>
                    <input type="file" id="image" name="image" required class="input">
                </div>
